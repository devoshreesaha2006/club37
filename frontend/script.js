...
function initJoinForm() {
    ...
    fileInput.addEventListener('change', () => {
      if (fileInput.files && fileInput.files[0]) {
        ...
        if (!validTypes.includes(file.type)) {
          showFieldError('profilePhoto', 'Only JPEG, PNG or WEBP images are allowed.');
          fileInput.value = '';
          return;
        }
  ...
  function validateClientSide(fd) {
    let valid = true;
    const required = ['name', 'age', 'city', 'phone', 'motorcycle', 'bikeModel', 'experience', 'reason'];
    ...
    if (reason && reason.length < 10) {
      showFieldError('reason', 'Please tell us a bit more (at least 10 characters).');
      valid = false;
    }
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      const data = await apiRequest('/applications', { method: 'POST', body: fd });
...
function initMembersPage() {
  ...
  grid.innerHTML = Array.from({ length: 6 })
    .map(() => `<div class="member-card"><div class="member-photo skeleton"></div><div class="member-info"><div class="skeleton" style="height:16px;width:70%;margin-bottom:8px;"></div><div class="skeleton" style="height:12px;width:50%;"></div></div></div>`)
    .join('');
...
function renderMembers(grid, members) {
  ...
  grid.innerHTML = members
    .map((m) => {
      const photo = m.profilePhoto && m.profilePhoto.url ? `style="background-image:url('${escapeHtml(m.profilePhoto.url)}')"` : '';
...
function initAdminDashboard() {
  ...
  function activateTab(tab) {
    ...
    sidebarLinks.forEach((b) => b.classList.toggle('active', b.dataset.gotoTab === tab));
    document.getElementById('members-panel').style.display = tab === 'MEMBERS' ? 'block' : 'none';
    document.getElementById('applications-panel').style.display = tab === 'MEMBERS' ? 'none' : 'block';
    if (tab !== 'MEMBERS') loadApplications(tab);
  }

  tabButtons.forEach((btn) => btn.addEventListener('click', () => activateTab(btn.dataset.tab)));
  sidebarLinks.forEach((btn) => btn.addEventListener('click', () => activateTab(btn.dataset.gotoTab)));
  ...
  async function loadApplications(status) {
    listEl.innerHTML = `<div class="skeleton" style="height:120px;margin-bottom:16px;"></div>`.repeat(3);
    try {
      const { applications } = await apiRequest(`/admin/applications?status=${status}`);
  ...
  function renderApplications(apps) {
    ...
    listEl.innerHTML = apps
      .map((a) => {
        const photo = a.profilePhoto && a.profilePhoto.url ? `style="background-image:url('${escapeHtml(a.profilePhoto.url)}');background-size:cover;"` : '';
  ...
  listEl.addEventListener('click', async (e) => {
    ...
    try {
      await apiRequest(`/admin/applications/${id}/${action}`, { method: 'POST' });
